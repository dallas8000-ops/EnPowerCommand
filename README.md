...
- **Leads** — company, role, URL, notes, stage.
- **Profile** — resume / highlights in browser storage (used for imports + outreach context).
...
- **Generate outreach** — drafts + subject lines; sends profile context when set.

## Quick start

1. **Postgres:** `docker compose up -d` (default port **5433** on host).
2. **Server env:** copy `server/env.example` → `server/.env`, set `DATABASE_URL`, run `npm run db:init --prefix server`.
...
5. **Shell** on the API service once (after Postgres is linked):
